...
air_service_key = '서비스키'
weather_service_key = '서비스키'
...
with open('air_data.json', 'rt', encoding='UTF-8') as air:
    ...
    for i in range(len(air_data['list'])):
        ...
        try:
            ...
            REH = REHToInt(weather_data['response']['body']['items']['item'][2]['category'])
            RN1 = RN1ToInt(weather_data['response']['body']['items']['item'][3]['category'])
            T1H = T1HToInt(weather_data['response']['body']['items']['item'][5]['category'])
            VEC = VECToInt(weather_data['response']['body']['items']['item'][7]['category'])
            WSD = WSDToInt(weather_data['response']['body']['items']['item'][9]['category'])
